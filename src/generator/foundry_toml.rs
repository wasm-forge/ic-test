...
pub fn add_contracts(setup: &mut IcTestSetup) -> anyhow::Result<()> {
    if !setup.evm_setup.skip_foundry_toml {
        use toml_edit::DocumentMut;

        let toml = fs::read_to_string("foundry.toml")?;

        let doc = toml
            .parse::<DocumentMut>()
            .expect("Failed to parse forge.toml");

        let src = get_toml_src(&doc);
        if let Some(path) = src {
            setup.evm_setup.foundry_src = path;
        }

        let out = get_toml_out(&doc);
        if let Some(path) = out {
            setup.evm_setup.foundry_out = path;
        }
    }

    // add all contracts from "src" to the setup
    let path = Path::new(&setup.evm_setup.foundry_src);
    if path.is_dir() {
